...
@torch.no_grad()
def split_num_cat_target(syn_data, info, cat_inverse):
    ...
    return syn_cat, syn_target

def recover_data(syn_cat, syn_target, info):
...
def sample(
    ...
    syn_cat, syn_target = split_num_cat_target(syn_data, info, cat_inverse) 
    syn_df = recover_data(syn_cat, syn_target, info)
